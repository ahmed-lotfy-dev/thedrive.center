...
import { useRandomAdvice } from "@/hooks/use-advice";
...
export function AdvicePopup({ delaySeconds = 30 }: AdvicePopupProps) {
  ...
  const { data: advice, isLoading } = useRandomAdvice();

  useEffect(() => {
    if (!advice || hasBeenDismissed || isLoading) return;
    const timer = setTimeout(() => setIsVisible(true), delaySeconds * 1000);
    return () => clearTimeout(timer);
  }, [advice, delaySeconds, hasBeenDismissed, isLoading]);

  if (hasBeenDismissed || isLoading || !advice) return null;
      ...
      {isVisible && (
        ...
        >
          <div className="relative group">
            <div className="absolute -inset-1 bg-linear-to-r from-emerald-500 to-teal-500 rounded-4xl blur opacity-25 group-hover:opacity-40 transition duration-1000 group-hover:duration-200" />
