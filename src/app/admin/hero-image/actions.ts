...
export async function updateHeroImageMobile(imageUrl: string) {
  try {
    await requireAdmin();
    await siteSettingQueries.set("hero_image_mobile_url", imageUrl);
    revalidatePath("/");
    revalidatePath("/admin/hero-image");
    logger.info("admin.hero_image_mobile.updated", {
      imageUrl,
    });
    return { success: true };
  } catch (error) {
    ...
    logger.error("admin.hero_image_mobile.update_failed", {
      error,
      imageUrl,
    });
    return { error: "فشل تحديث صورة الموبايل" };
  }
}
