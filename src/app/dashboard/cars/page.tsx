  ...
  return (
    <div dir="rtl" className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-slate-900 dark:text-white">سجل السيارات</h1>
          <p className="text-slate-500 dark:text-slate-400 mt-1">ابحث عن سيارة برقم اللوحة أو الماركة لعرض تاريخ الصيانة</p>
        </div>
      </div>
        ...
        <form className="flex gap-3 max-w-2xl">
          <div className="relative flex-1">
            ...
            <Input 
              ...
              className="pl-4 pr-10 h-12 bg-slate-50 dark:bg-slate-800/50 uppercase"
            />
          </div>
          <Button type="submit" className="h-12 px-8 bg-blue-600 hover:bg-blue-700">بحث</Button>
        </form>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {carsWithHistory.map((car) => (
          <div key={car.id} className="bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-md transition-shadow group relative overflow-hidden">
             {/* Decorative background accent */}
            <div className="absolute top-0 right-0 w-32 h-32 bg-blue-500/5 rounded-full blur-3xl -mr-10 -mt-10" />
            
            <div className="flex items-start justify-between relative z-10">
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 rounded-2xl bg-blue-50 dark:bg-blue-900/20 flex items-center justify-center border border-blue-100 dark:border-blue-900/50">
                  <CarFront className="w-6 h-6 text-blue-600 dark:text-blue-400" />
                </div>
                <div>
                  <h3 className="font-bold text-lg text-slate-900 dark:text-white uppercase">
                    {car.plateNumber}
                  </h3>
            ...
            <div className="mt-6 pt-6 border-t border-slate-100 dark:border-slate-800/60 grid grid-cols-2 gap-4 relative z-10">
              <div>
                <p className="text-xs text-slate-500 mb-1">عدد الزيارات</p>
              ...
              <div>
                <p className="text-xs text-slate-500 mb-1">آخر زيارة</p>
                <div className="flex items-center gap-2 font-semibold text-slate-700 dark:text-slate-300">
            ...
            <div className="mt-6 w-full flex items-center justify-center gap-2 py-3 bg-slate-50 dark:bg-slate-800/50 text-blue-600 dark:text-blue-400 font-medium rounded-xl group-hover:bg-blue-600 group-hover:text-white transition-colors relative z-10 pointer-events-none">
        ...
        {carsWithHistory.length === 0 && (
          <div className="col-span-full py-20 text-center">
            ...
            <h3 className="text-lg font-bold text-slate-900 dark:text-white">لا توجد سيارات مطابقة</h3>
