  ...
  return (
    <>
      <CardHeader className="text-center pb-4">
        <CardTitle className="text-3xl font-black tracking-tight text-foreground bg-clip-text text-transparent bg-gradient-to-l from-emerald-600 to-cyan-600 dark:from-emerald-400 dark:to-cyan-400">تـسـجيـل الـدخول</CardTitle>
      </CardHeader>
      ...
      <CardContent className="px-6 md:px-10 pb-8 space-y-8">
        ...
        <div className="space-y-4">
          <Button 
            variant="outline" 
            className="w-full h-14 rounded-2xl border-emerald-500/30 hover:bg-emerald-500/5 hover:border-emerald-500 transition-all flex items-center justify-center gap-4 text-lg font-bold group bg-white/5 dark:bg-black/20 backdrop-blur-sm shadow-lg shadow-emerald-500/5"
            onClick={async () => {
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5">
            ...
            />
            <Button 
              className="w-full h-14 rounded-2xl bg-foreground hover:bg-foreground/90 text-background font-black text-lg shadow-xl shadow-foreground/5 transition-all active:scale-[0.98] mt-2 group" 
              type="submit" 
              disabled={isSocialLoading || isEmailLoading}
            >
      ...
      <CardFooter className="flex flex-col items-center gap-4 py-6 border-t border-border/40 bg-slate-50/30 dark:bg-slate-900/30">
        <Button variant="link" onClick={onSwitch} className="text-muted-foreground hover:text-emerald-500 transition-colors font-bold text-base">
